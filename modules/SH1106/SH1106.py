import modules.SH1106.config
import RPi.GPIO as GPIO
import time
import numpy as np

Device_SPI = modules.SH1106.config.Device_SPI
Device_I2C = modules.SH1106.config.Device_I2C
...
class SH1106(object):
    def __init__(self):
        ...
        self._dc = modules.SH1106.config.DC_PIN
        self._rst = modules.SH1106.config.RST_PIN
        self._bl = modules.SH1106.config.BL_PIN
        self.Device = modules.SH1106.config.Device
    ...
    def command(self, cmd):
        if(self.Device == Device_SPI):
            ...
            modules.SH1106.config.spi_writebyte([cmd])
        else:
            modules.SH1106.config.i2c_writebyte(0x00, cmd)
    ...
    def Init(self):
        if (modules.SH1106.config.module_init() != 0):
            return -1
        """Initialize dispaly"""    
        self.reset()
        ...
        self.command(0xDA);#--set com pins modules.SH1106 configuration
        self.command(0x12);
    ...
    def ShowImage(self, pBuf):
        for page in range(0,8):
            ...
            for i in range(0,self.width):#for(int i=0;i<self.width; i++)
                if(self.Device == Device_SPI):
                    modules.SH1106.config.spi_writebyte([~pBuf[i+self.width*page]]); 
                else :
                    modules.SH1106.config.i2c_writebyte(0x40, ~pBuf[i+self.width*page])
